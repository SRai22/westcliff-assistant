"""
Test script for OpenAI client implementation.

Tests basic functionality, retry logic, and ensures no API keys are logged.
Run with: python -m pytest tests/test_openai_client.py
Or directly: python tests/test_openai_client.py
"""
import asyncio
import logging
import sys
...
async def test_openai_client():
    """Test the OpenAI client with a simple prompt."""
    ...
    print("\n" + "="*60)
    print("Testing OpenAI client with structured output...")
    print("="*60)
...
async def test_schema_validation():
    ...
    Note: With OpenAI's strict structured outputs, validation errors are rare,
    but this demonstrates the retry mechanism exists.
    """
    print("\n" + "="*60)
    print("Schema validation test (retry mechanism)")
    print("="*60)
    print("Note: OpenAI's structured outputs ensure schema compliance,")
    print("so validation errors are unlikely in normal operation.")
    print("The retry logic handles edge cases and other API errors.")
